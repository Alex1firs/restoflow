...
| Connect drop-off map pin | 🟡 | Coordinates are captured explicitly and are authoritative: device location, or pasted coordinates. A quote is impossible without them | **No map provider is configured in RestoFlow and none was added** — an embedded pin needs a provider decision (free OSM/Leaflet vs paid Google Maps) | **Owner decision** |
