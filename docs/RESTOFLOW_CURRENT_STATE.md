...
| Connect merchant UI | 🟡 | `/admin/[slug]/connect` inside the existing admin, gated on the capability — board of active/recent deliveries, New Delivery form, quote, payer choice, copy/share link, detail page with the lifecycle, pickup code at the right stage. Nav entry appears only for Connect restaurants. Both screens poll the existing endpoints every 8s; **no second realtime system**. Verified server-side on staging: renders, gates (non-Connect tenant → **404**, cross-tenant → **307** to own), no Dispatcher vocabulary, no cost/margin/receiving code | **Interactive browser pass not done** — driving the authenticated UI needs a login I cannot perform (entering a password is out of bounds, and injecting a session cookie was blocked) | Owner to log in, or grant the browser permission |
| Connect drop-off map pin | ✅ | **Leaflet + OpenStreetMap**, no API key, attribution rendered (licence condition). Tap to place, drag to fine-tune, "use my location"; map opens on the restaurant's own pin. Coordinates stay authoritative and **staff never see or type them** — the coordinate field is gone. Loaded on demand, so no other admin page ships Leaflet | — | — |
| Connect address autocomplete | ⬜ | **Deliberately absent.** Typing an address does not move the pin. OSM's public Nominatim is not a production geocoding service — it would work in testing and rate-limit in the evening rush | A real geocoding provider, chosen on its own merits | **Future decision** |
| Connect cancellation | ⬜ | Not started. `cancelDelivery()` exists on the client and is called by nothing — unimplemented for the marketplace too | Shared work for both products | — |
