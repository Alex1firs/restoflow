...
| Connect billing — pay per delivery | 🟡 | **No wallet, by decision.** Payer chosen per delivery (`customer` \| `restaurant`); platform-collected Paystack, no subaccount split; dispatch happens behind the webhook so a courier is never engaged against uncleared money. Verified on staging up to the checkout: quote → payer choice → frozen amount → guest link; near-expiry quote refreshed before Paystack; server-controlled amount; wrong token → 404 | **A real Paystack TEST payment has not yet been completed**, so webhook-credit-once, single-job-on-payment, replay safety, refund execution and the pay→track transition are unproven end to end | Owner to complete one test checkout |
