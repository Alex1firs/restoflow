...
| **6.3 Environment integrity** | Staging/production isolation, automated deploys, required environment variables present. **Open:** port the rider-eligibility enforcement to the production delivery service — until then the app's fallback is what protects production, and the two must ship in that order. |
