...
import AdminPreparedItemsClient from "./AdminPreparedItemsClient";
...
export default async function AdminPreparedItemsPage({ params }: Props) {
  ...
  if (user.restaurantSlug !== slug) {
    redirect(`/admin/${user.restaurantSlug}/prepared-items`);
  }
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 min-w-0 pt-14 pb-16 lg:pt-0 lg:pb-0">
        ...
        <AdminPreparedItemsClient restaurant={restaurant} aiEnabled={!!process.env.GEMINI_API_KEY} />
      </div>
    </div>
  );
}
