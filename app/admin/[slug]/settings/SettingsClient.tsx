...
export default function SettingsClient({ restaurant }: Props) {
  ...
  async function handleWhatsAppTest() {
    if (!form.whatsappPhone.trim()) {
      setTestStatus("error");
      setTestError("Enter a WhatsApp phone number first.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Server saved whatsappEnabled: true and normalized the phone
        ...
        <Section title="Notifications">
            ...
            <div className="border border-gray-100 rounded-2xl p-5 space-y-4 bg-gray-50">
              <div className="flex items-center gap-2">
                ...
                {form.whatsappEnabled && (
                  <span className="text-xs font-bold bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Enabled</span>
                )}
              </div>
              ...
              <Field label="WhatsApp Number">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  International format preferred (e.g. 2348012345678). Nigerian local format (08012345678) is auto-converted.
                </p>
              </Field>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {testStatus === "success" && (
                  <span className="text-sm font-bold text-green-600 flex items-center gap-1.5">
                    ...
                    Test sent — WhatsApp alerts enabled!
                  </span>
                )}
                {testStatus === "error" && (
              ...
              {/* Enable toggle (manual override after test) */}
          ...
          {(form.alertPreference === "sms" || form.alertPreference === "both") && (
            <Field label="SMS Alert Number">
              ...
              />
              <p className="text-xs text-gray-400 mt-1">
                Receive an SMS when a new order arrives. Requires TERMII_API_KEY on the server.
              </p>
            </Field>
          )}
        </Section>
